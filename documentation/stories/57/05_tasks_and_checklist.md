...
**Files changed:** `packages/dashboard/app/(dashboard)/dashboard/folders/page.tsx` (full rewrite), `packages/dashboard/app/api/folders/route.ts` (full rewrite), `packages/dashboard/next.config.ts`, `packages/dashboard/package.json`.

**Tested:** Real, authenticated `curl` session (logged in as `pawel_f`/`changeme` against a locally-running `next dev` + the real, already-running .NET Content Provider API on port 12024) — `GET /api/folders` returned the real repo root item (`Config.name: "chad_pawel_f"`, real children). `GET /dashboard/folders` returned HTTP 200 with the expected `Repo:` label text present in the rendered HTML shell, no server error in the Next.js dev log. **Not verified**: an actual mouse click on the buttons in a real browser — no headless-browser/screenshot tool is available in this environment (same limitation noted in the earlier Content Provider TS rewrite session's `cp-gui` work).
