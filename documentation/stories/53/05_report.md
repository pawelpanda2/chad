...
- **Full real browser click-through** (headless Chrome via
  ...
  15. Content check: the Views preview panel showed "Empty content" —
      **but** directly verified (separate `GetItem` call to the Content
      ...
      `06/propositions.md`, not fixed (out of scope: a generic shared-component
...
of scope, logged in `06/propositions.md`.
...
- Trzy testowe raporty pozostały w danych lokalnego Content Providera (nie
  da się ich usunąć — brak operacji delete). Patrz `06/propositions.md`.
- `PreviewContent`/`HeadersRenderer` pokazuje "Empty content" dla zwykłego
  tekstu bez nagłówków (pre-istniejące, nienaprawione — patrz `06/propositions.md`).
- Naming scheme `views/daily` (data+litera zamiast liczb) — zgłoszone przez
  użytkownika w trakcie realizacji, świadomie odłożone (patrz `06/propositions.md`).
- `cp-flow.ts` zawiera martwe, zduplikowane funkcje dla dates/daily —
  literały zaktualizowane, funkcje nie usunięte (patrz `06/propositions.md`).
...
Patrz `06/propositions.md` (5 pozycji, żadna nie blokuje zamknięcia Story
