 ...
 * Gets leads statuses.
 * 
 * Implements the working C# test approach exactly:
 * 1. IItemWorker.PostByNames(repoId, "Folder", "leads", "all items") → get allItems with loca
 * 2. IManyItemsWorker.GetManyByName(repoId, allItems.AdrTuple.Loca, "status") → get statuses
 * 
 * IMPORTANT: Use IManyItemsWorker (interface), NOT ManyItemsWorker (implementation).
 * 
 * @returns An array of status items (each with Settings.address and Body)
 */
export async function chad_GetLeadsStatuses(): Promise<any> {
  // Step 1: PostByNames to get "leads/all items" folder with loca (C# test: IItemWorker.PostByNames)
  ...
  if (!allItems?.Settings?.address) {
    throw new Error(
      "chad_GetLeadsStatuses: Could not resolve leads loca - no Settings.address in PostByNames response"
    );
  }
