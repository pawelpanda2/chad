 ...
 * pre-seeding a collision non-deterministic): the metadata sidecar write
 * failing right after the photo bytes already landed (must clean up the
 * orphan file), and the file-delete step of `deleteContactPhoto` failing
 * (must abort before touching metadata, per the module's documented
 * explicit-compensation contract).
 */
...
const { saveContactPhoto, deleteContactPhoto, getUserContactPhotosDir } = await import(
  "./google-contact-photos.js"
);
...
describe("saveContactPhoto — metadata write failure", () => {
  let rootDir: string;

  beforeEach(async () => {
    rootDir = await mkdtemp(path.join(tmpdir(), "chad-contact-photos-fail-"));
    ...
    await expect(
      runWithRepoContext(USER, () =>
        saveContactPhoto({
          ...
          contactResourceName: "people/c123",
...
describe("deleteContactPhoto — file-delete failure", () => {
  let rootDir: string;
  ...
  it("aborts before deleting metadata when removing the photo file fails, leaving both intact for a retry", async () => {
    ...
    await expect(
      runWithRepoContext(USER, () => deleteContactPhoto(saved.id, { rootDirectory: rootDir })),
    ).rejects.toMatchObject({ code: "WRITE_FAILED" });
