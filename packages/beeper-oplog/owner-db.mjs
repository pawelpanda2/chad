 ...
 * validated, no default. beeper-oplog has no Dashboard session to read a
 * repoGuid from, so the owner must be configured explicitly in this
 * container's env. Story 73: each CHAD user has their own
 * `beeper_<repoGuid>` MongoDB database — this process must never guess a
 * user, never fall back to any default, and never touch the old shared
 * `beeper` database.
 *
  ...
  if (!repoGuid || !GUID_RE.test(repoGuid)) {
    console.error(
      `[owner-db] BEEPER_OWNER_REPO_GUID is missing or invalid (${JSON.stringify(repoGuid)}). ` +
        ...
        "so BEEPER_OWNER_REPO_GUID must be set to a full GUID in this container's env."
    );
    process.exit(1);
  }
  return repoGuid;
