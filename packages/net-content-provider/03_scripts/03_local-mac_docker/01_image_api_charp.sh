...
# ============================================
# Build obrazu cp_webapi z informacjami o Git
# ============================================
...
echo "🚀 docker buildx build --platform $DETECTED_PLATFORM -f $DOCKERFILE_DIR/webapi -t $FULL_IMAGE_NAME --build-arg GIT_COMMIT=$GIT_COMMIT --build-arg GIT_COMMIT_SHORT=$GIT_COMMIT_SHORT --build-arg BUILD_DATE=$BUILD_DATE --build-arg GIT_REMOTE_URL=$GIT_REMOTE_URL --load $REPO_ROOT"

docker buildx build \
  --platform "$DETECTED_PLATFORM" \
  -f "$DOCKERFILE_DIR/webapi" \
  -t "$FULL_IMAGE_NAME" \
