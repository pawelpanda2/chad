...
public class GuidGroupsHelper
{
        ...
        List<string> searchFolders)
    {
        ...
        foreach (var searchFolder in searchFolders)
        {
            var reposFolder = Path.Combine(searchFolders.First(), "repos");
            var found = Directory.Exists(reposFolder);

            if (!found) { continue; }
            
            var possibleGuidFolders = Directory.GetDirectories(reposFolder);
            
            // var guidFolderList = new List<string>();
            foreach (var possibleGuidFolder in possibleGuidFolders)
            {
                if (IsUniRepoGroupFolder(possibleGuidFolder))
                {
                    if (!dict.ContainsKey(reposFolder))
                    {
                        dict.Add(reposFolder, new List<string>());
                    }
                    dict[reposFolder].Add(possibleGuidFolder);
                }
            }
        }
