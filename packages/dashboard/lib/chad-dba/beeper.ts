  ...
  try {
    ...
    for (const [channelKey, channelName] of Object.entries(beeperBody)) {
      ...
      if (foundEntry) {
        ...
        // Step 5: Build final address and get the conversation
        const finalAddress = joinAddress(mediaAddress, foundKey);
        const { repo: finalRepo, loca: finalLoca } = parseAddressToRepoLoca(finalAddress);
        
        const conversationItem = await invokeContentProvider([
          "IRepoService",
          "IItemWorker",
          "GetItem",
          finalRepo,
        ...
        if (conversationItem?.Body) {
          const body = typeof conversationItem.Body === "string" 
            ? conversationItem.Body 
            : JSON.stringify(conversationItem.Body);
        ...
        return {
          ...
          address: finalAddress,
          channel: channelName || channelKey,
          error: "Conversation item found but has no body",
        };
      }
    }
