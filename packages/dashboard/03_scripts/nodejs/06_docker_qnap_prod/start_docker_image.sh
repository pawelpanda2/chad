...
DEFAULT_CONTENT_PROVIDER_API_PORT="${CONTENT_PROVIDER_API_PORT:-12034}"
...
check_and_stop_running_container() {
	...
	if "${DOCKER_COMPOSE_CMD[@]}" -p "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE_PATH}" ps --format json 2>/dev/null | grep -q '"Running":true'; then
		echo_warn "Kontener jest już uruchomiony. Zatrzymywanie..."
		"${DOCKER_COMPOSE_CMD[@]}" -p "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE_PATH}" down --remove-orphans
...
main() {
	...
	echo_info "========================================"
	echo_info "Personal Dashboard - Start Docker Container (QNAP Prod)"
	echo_info "========================================"
	echo_info ""
	...
	# Export prod ports for compose interpolation
	export PROD_FRONTEND_PORT="${DEFAULT_FRONTEND_PORT}"
	export PROD_BACKEND_PORT="${DEFAULT_BACKEND_PORT}"
	...
	echo_info "Frontend port: ${PROD_FRONTEND_PORT}"
	echo_info "Backend port: ${PROD_BACKEND_PORT}"
	...
	# Check if port is in use
	if ! check_port_in_use "${PROD_FRONTEND_PORT}"; then
		echo_error "Cannot start container - port ${PROD_FRONTEND_PORT} is already in use."
		exit 1
	fi

	# Check if Content Provider API port is in use
	if ! check_port_in_use "${DEFAULT_CONTENT_PROVIDER_API_PORT}"; then
		echo_error "Cannot start container - port ${DEFAULT_CONTENT_PROVIDER_API_PORT} is already in use."
		exit 1
	fi

	echo_info "Uruchamianie kontenera..."
	echo_info "Frontend: http://${DEFAULT_QNAP_PUBLIC_HOST}:${PROD_FRONTEND_PORT}"
	echo_info "Content Provider API: http://${DEFAULT_QNAP_PUBLIC_HOST}:${DEFAULT_CONTENT_PROVIDER_API_PORT}"
	echo_info ""

	if [[ "${run_mode}" == "detach" ]]; then
		"${DOCKER_COMPOSE_CMD[@]}" -p "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE_PATH}" up --no-build -d
		echo_info "Kontener uruchomiony w tle."
		echo_info "Aby zobaczyć logi: ${DOCKER_COMPOSE_CMD[*]} -p ${COMPOSE_PROJECT_NAME} -f ${COMPOSE_FILE_PATH} logs -f"
		echo_info "Aby zatrzymać: ${DOCKER_COMPOSE_CMD[*]} -p ${COMPOSE_PROJECT_NAME} -f ${COMPOSE_FILE_PATH} down"
	else
		"${DOCKER_COMPOSE_CMD[@]}" -p "${COMPOSE_PROJECT_NAME}" -f "${COMPOSE_FILE_PATH}" up --no-build
	fi
}
