...
export default function DailyTrackerHistoryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/content-provider/daily-history?${params.toString()}`
      );
      ...
      setError(message);
      toast.error("Failed to fetch daily tracker history");
      console.error("[daily-history] fetch failed:", err);
    } finally {
      setIsLoading(false);
    }
  }, [page, pageSize]);
        ...
        {isLoading ? (
          ...
          <div className="flex items-center gap-3 py-4 text-muted-foreground">
            ...
            <span className="text-sm">
              No changes recorded yet. Your Daily Tracker is new.
            </span>
          </div>
        ) : (
