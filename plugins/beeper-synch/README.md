...
Story 91. Mac-only supervisor process for the existing Beeper sync
pipeline. **Not a new sync engine** — it spawns and supervises the packages
that already do the real work:

- `packages/beeper-ws` — long-lived WebSocket listener, Beeper Desktop ->
  `beeper_events`. Supervised here: spawned, restarted with bounded
  exponential backoff on unexpected exit, stopped gracefully on shutdown.
- `packages/beeper-sync` — one-shot incremental REST importer (respects
  Include/Exclude via its own `lib/sync-permissions.mjs`, writes
  `contacts`/`channels`/`messages`). Scheduled here on a fixed interval
  (`BEEPER_SYNCH_SYNC_INTERVAL_MS`), one run at a time.
